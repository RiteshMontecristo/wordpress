...
function mji_online_settings_page()
{
    ...
    if (isset($_POST['mji_online_settings_nonce']) && wp_verify_nonce($_POST['mji_online_settings_nonce'], 'mji_save_online_settings')) {
        update_option('mji_online_salesperson_id', absint($_POST['mji_online_salesperson_id'] ?? 0));
        update_option('mji_notification_email', sanitize_email($_POST['mji_notification_email'] ?? ''));
        echo '<div class="notice notice-success"><p>Settings saved.</p></div>';
    }

    $saved_salesperson = (int) get_option('mji_online_salesperson_id', 0);
    ...
    <div class="wrap">
        ...
        <form method="post">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="mji_online_salesperson_id">Default Salesperson</label></th>
                    <td>
                        <select name="mji_online_salesperson_id" id="mji_online_salesperson_id" required>
                            <option value="">— Select —</option>
                            <?php foreach ($salespeople as $s): ?>
                                <option value="<?php echo esc_attr($s->id); ?>" <?php selected($s->id, $saved_salesperson); ?>>
                                    <?php echo esc_html($s->first_name . ' ' . $s->last_name); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <p class="description">Used as the salesperson on all MJI orders created from online sales.</p>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <p class="description">Who receives the inventory-updated / error emails when an online order is placed. Defaults to the WordPress admin email if left blank.</p>
                    </td>
                </tr>
            </table>
...
function adjust_stock_after_order($order_id, $posted_data, $order)
{
    ...
    $salesperson_id = (int) get_option('mji_online_salesperson_id', 0);
    if (!$salesperson_id) {
        custom_log("Online order #{$order_id}: mji_online_salesperson_id not configured — skipping MJI inventory sync.");
        mji_notify_online_sale_error($order, 'mji_online_salesperson_id is not configured in MJI Online Settings. Please set it, then manually create this sale in the inventory system.');
        return;
    }
    ...
    $sold_units = []; // each entry: ['unit' => row, 'product_name' => string, 'unit_price' => float]
    ...
    try {
        ...
        // GST/HST → gst_total; PST/QST → pst_total.
        // This correctly handles inter-provincial orders (AB = GST only, ON = HST only, QC = GST+QST).
        $gst_total = 0.0;
        $pst_total = 0.0;
        foreach ($order->get_taxes() as $tax_item) {
            $label = strtolower($tax_item->get_name());
            if (str_contains($label, 'pst') || str_contains($label, 'qst')) {
        ...
        // Insert order items and flip inventory status
        foreach ($sold_units as $entry) {
            $unit       = $entry['unit'];
