...
function mji_enqueue_admin_scripts(string $hook): void
{
    ...
    // WC product edit page — loads the Inventory Units meta box
    ...
    $inventory_hooks = [
        ...
        'inventory-management_page_items-management',
        'inventory-management_page_customer-management',
        'inventory-management_page_salespeople-management',
        'inventory-management_page_invoice-management',
        'inventory-management_page_reports-management',
    ];

    if (!in_array($hook, $inventory_hooks, true)) {
        return;
    }

    // Scripts — all inventory pages
    ...
    $page_styles = [
        ...
        'inventory-management_page_items-management'    => ['items-style',    'items.css'],
        'inventory-management_page_customer-management' => ['customer-style', 'customer.css'],
        'inventory-management_page_invoice-management'  => ['find-style',     'find_invoice.css'],
        'inventory-management_page_reports-management'  => ['reports-style',  'reports.css'],
    ];

    if (isset($page_styles[$hook])) {
        [$handle, $file] = $page_styles[$hook];
        wp_enqueue_style($handle, $dir . '/inventory/styles/' . $file, [], filemtime($path . '/inventory/styles/' . $file));
    }
}
